...
const maxJackpotChance = 0.20; // Limite máximo de 20% para a chance do jackpot

function getRandomSymbol() {
    const symbols = ['🍒', '🍋', '🔔', '⭐', '💎', '7️⃣'];
    const baseWeights = [0.25, 0.25, 0.25, 0.10, 0.10, 0.05]; // chances basicas
...
function pullLever() {
    ...
    setTimeout(() => {
        ...
        if (reel1.textContent === reel2.textContent && reel2.textContent === reel3.textContent) {
            ...
            loseSound.play();
            // Aumenta a chance extra de jackpot em 1% (0.01) a cada falha, até o limite máximo. Evite fazer mudanças Leandro do futuro ;)
            extraJackpotChance = Math.min(extraJackpotChance + 0.01, maxJackpotChance);
        }
    }, 3500);
}
